...
class Register extends Component {
  ...
  submitRegister = async () => {
    ...
    if (email.length < 4) this.setState({ error: 'Email must be at least 4 characters.' });
    ...
    //else if (!dob.match(/^\d{4}-\d{2}-\d{2}$/g)) this.setState({ error: 'Must provide DOB in YYYY-MM-DD form.' });
      ...
      //Api.register(email, password, first, last, dob, this.callback);
      try {
        await firebaseAuth
          .auth()
